...
class CelebaHQDataset(Dataset):
    def __init__(self, split="train", transform=None, size=256):
        # 直接用datasets库加载
        self.dataset = load_dataset("mattymchen/celeba-hq", split=split)
        self.size = size
    ...
    def __getitem__(self, idx):
        ...
        # 如果字段是 image.bytes
        image_bytes = data_i.get("image.bytes", None)
        if image_bytes is not None:
            image = Image.open(io.BytesIO(image_bytes)).resize((self.size, self.size))
        else:
            # 如果字段是 image（PIL Image 或路径）
            image = data_i["image"]
            if not isinstance(image, Image.Image):
                image = Image.open(image).resize((self.size, self.size))
            else:
...
def main_lightning():
    ...
    train_dataset = CelebaHQDataset(split="train", transform=train_transform)
    val_dataset = CelebaHQDataset(split="validation", transform=train_transform)
